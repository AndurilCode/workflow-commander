        ...
        if service_type in self._singletons:
            if self._singletons[service_type] is None:
                ...
                if service_type not in self._factories:
                    raise DependencyInjectionError(f"No factory registered for singleton service: {service_type}")
...
def inject_config_service(func: Callable) -> Callable:
    ...
    """
    def wrapper(*args, **kwargs):
        from .config_service import ConfigurationService, get_configuration_service
