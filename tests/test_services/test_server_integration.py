...
from src.dev_workflow_mcp.services.dependency_injection import clear_registry, get_service
...
class TestServerConfigurationIntegration:
    ...
    def test_server_configuration_creation_with_custom_args(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            args = parser.parse_args([
                ...
                "--session-retention-hours", "24",
                "--disable-session-archiving",
                "--enable-cache-mode",
                "--cache-db-path", "/custom/cache",
                "--cache-collection-name", "custom_collection",
                "--cache-embedding-model", "custom-model",
                "--cache-max-results", "100",
            ])
    ...
    def test_server_startup_with_cache_mode(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            test_args = [
                ...
                "--repository-path", temp_dir,
                "--enable-cache-mode",
                "--cache-max-results", "75",
            ]
            
            with patch('sys.argv', test_args):
                with patch('src.dev_workflow_mcp.utils.session_manager.auto_restore_sessions_on_startup') as mock_restore:
    ...
    def test_server_startup_configuration_error(self, mock_fastmcp):
        ...
        test_args = [
            "server.py",
            "--repository-path", "/nonexistent/path/that/does/not/exist",
        ]
            ...
            with patch('sys.argv', test_args):
                with patch('src.dev_workflow_mcp.server.FastMCP'):
                    with patch('src.dev_workflow_mcp.server.register_phase_prompts'):
                        with patch('src.dev_workflow_mcp.server.register_discovery_prompts'):
                            result = main()
    ...
    def test_backward_compatibility_legacy_config(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('sys.argv', test_args):
                with patch('src.dev_workflow_mcp.server.FastMCP'):
                    with patch('src.dev_workflow_mcp.server.register_phase_prompts') as mock_register_phase:
                        with patch('src.dev_workflow_mcp.server.register_discovery_prompts') as mock_register_discovery:
                            result = main()
